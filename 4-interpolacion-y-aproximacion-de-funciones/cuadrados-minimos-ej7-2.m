...
vector_y = [5.0, 15.3, 29.3, 46.4, 66.3, 80.2];

### Estimar la recta de máximo ajuste

# Calculo los valores de los elementos de las matrices
sum_x4 = sum(vector_x .* vector_x .* vector_x .* vector_x);
sum_x3 = sum(vector_x .* vector_x .* vector_x);
sum_x2 = sum(vector_x .* vector_x);
sum_x = sum(vector_x);
n = length(vector_x);

sum_x2y = sum(vector_x .* vector_x .* vector_y);
...
matrix_1 = [sum_x4, sum_x3, sum_x2; sum_x3, sum_x2, sum_x; sum_x2, sum_x, n];
matrix_2 = [sum_x2y; sum_xy; sum_y];
...
printf('Coeficientes resultantes (a, b y c) \n\n');
disp(result);
printf('\n\n');
